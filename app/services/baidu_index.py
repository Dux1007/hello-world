...
class BaiduIndexClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.default_proxies: Dict[str, str] = self._normalize_proxies(proxies)
    ...
    def update_proxies(self, proxies: Optional[Dict[str, str]]) -> None:
        """Update the default proxy configuration used for HTTP requests."""

        self.default_proxies = self._normalize_proxies(proxies)
    ...
    def fetch_indices(
        ...
        *,
        ...
        proxies_override: Optional[Dict[str, str]] = None,
    ) -> List[DailySeries]:
    ...
    def _fetch_series(
        ...
        *,
        ...
        ptbk = self._fetch_ptbk(uniqid=uniqid, cookie=cookie, proxies=proxies)
    ...
    def _fetch_ptbk(self, *, uniqid: str, cookie: str, proxies: Optional[Dict[str, str]]) -> str:
        self._sleep_before_request()
        response = self._safe_get(
            self.PTBK_ENDPOINT,
    ...
    def _safe_get(
        ...
        *,
        ...
        try:
            ...
            resolved_proxies = self._resolve_proxies(proxies)
            if resolved_proxies:
                request_kwargs["proxies"] = resolved_proxies
    ...
    # Proxy helpers ------------------------------------------------------------
    @staticmethod
    def _normalize_proxies(proxies: Optional[Dict[str, str]]) -> Dict[str, str]:
        if not proxies:
            return {}
    ...
    def _resolve_proxies(self, override: Optional[Dict[str, str]]) -> Dict[str, str]:
        resolved: Dict[str, str] = {}
        if self.default_proxies:
            resolved.update(self.default_proxies)
        override_normalized = self._normalize_proxies(override)
        if override_normalized:
            resolved.update(override_normalized)
        return resolved
