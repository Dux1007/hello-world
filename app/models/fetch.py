...
class FetchRequest(BaseModel):
    ...
    proxy: Optional[str] = Field(
        default=None,
        description="Proxy address applied to both HTTP and HTTPS requests (e.g. http://127.0.0.1:7890).",
    )
    http_proxy: Optional[str] = Field(
        default=None,
        description="HTTP-specific proxy override (takes precedence over `proxy`).",
    )
    https_proxy: Optional[str] = Field(
        default=None,
        description="HTTPS-specific proxy override (takes precedence over `proxy`).",
    )
